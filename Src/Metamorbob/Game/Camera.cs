...
namespace GameManager
{
  public static class Camera
  {
    ...
    public static void Follow(Sprite actor = null, FollowingType followType = FollowingType.LOCKON, float lerp = 1f)
    {
      ...
      switch (Camera.FollowingType)
      {
        ...
        case FollowingType.ALWAYS_ON_RIGHT:
          Camera.Position = new Vector2(actor.Position.X - ((float) Game1.WIDTH - (float) actor.Width * actor.Scale.X), actor.Position.Y - ((float) Game1.HEIGHT - (float) actor.Height * actor.Scale.Y));
          break;
      }
    ...
    {
      if (Camera.DEBUG_IS_ON)
        Game1.Instance.spriteBatch.Draw(Camera.CameraDebugArea, new Vector2((float) Camera.VisibleArea.X, (float) Camera.VisibleArea.Y), Color.Multiply(Color.White, 0.4f));
      if (Camera.canFlash)
        Primitive.DrawRectangle(Primitive.PrimitiveStyle.FILL, Game1.Instance.spriteBatch, (float) (Camera.VisibleArea.X - 4 * Camera.VisibleArea.Width), (float) (Camera.VisibleArea.Y - 4 * Camera.VisibleArea.Height), 10 * Camera.VisibleArea.Width, 10 * Camera.VisibleArea.Height, Color.Multiply(Camera.flashColor, Camera.alphaFlash));
      if (!Camera.canFade)
        return;
      Primitive.DrawRectangle(Primitive.PrimitiveStyle.FILL, Game1.Instance.spriteBatch, (float) (Camera.VisibleArea.X - 4 * Camera.VisibleArea.Width), (float) (Camera.VisibleArea.Y - 4 * Camera.VisibleArea.Height), 10 * Camera.VisibleArea.Width, 10 * Camera.VisibleArea.Height, Color.Multiply(Camera.fadeColor, Camera.alphaFade));
    }

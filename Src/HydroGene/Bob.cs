    ...
    private void CheckCollision()
    {
      ...
      if (this.IsStanding || (double) this.Velocity.Y > 0.0)
      {
        ...
        if (this.collide)
        {
          if (!this.IsStanding)
          {
            AssetManager.Sound_Landing.SoundEffect.Play(MainGame.VOLUME_SFX, 0.0f, 0.0f);
            this.CanJump = true;
    ...
    public override void Update(GameTime gameTime)
    {
      ...
      if (!this.IsDead)
      {
        if (this.CurrentState != Bob.State.CANON)
        {
          ...
          {
            AssetManager.Sound_Jump.SoundEffect.Play(MainGame.VOLUME_SFX, 0.0f, 0.0f);
            if (this.CurrentMode == Bob.Mode.NORMAL)
            {
        ...
        if (this.CurrentMode == Bob.Mode.CANON)
        {
          ...
          {
            AssetManager.Sound_Shoot.SoundEffect.Play(MainGame.VOLUME_SFX, 0.0f, 0.0f);
      ...
      if (this.IsDead)
      {
        ...
        {
          ++MainGame.NB_DEATH[(int) MainGame.CURRENT_LEVEL - 1];
